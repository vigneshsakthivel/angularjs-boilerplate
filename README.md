...
A modified version of [ngBoilerPlate](https://github.com/ngbp/ngbp) and is used as kickstarter for AngularJS projects by LAMP-SL in [Aspire Systems India Pvt Ltd](https://aspiresys.com).
...
`angularjs-boilerplate` is modified version of [ngBoilerPlate](https://github.com/ngbp/ngbp).
Following are the modifications made to the ngBoilerPlate.
- Foler structure of the application. 
- Version of packages used. Most of them have been upgraded to the lastest version
- Removed support for less, coffee. If required can be added in future.
- Added CSS Minification process

angularjs-boilerplate also comes prepackaged with the most popular design frameworks
...
ng-boilerplate/
  ...
  |  |  |- <static files>
  |  |- common/
  |  |  |- <reusable / helper js code>
  |  |- components/
  |  |  |- <reusable angularjs code - directives, filters, service, etc>  
  |- vendor/
  |  |- angular-bootstrap/
  |  |- bootstrap/
  |- .bowerrc
...
What follows is a brief description of each entry, src folder contain
its  own `README.md` file with additional documentation
...
getting `angularjs-boilerplate` up and running. Though `angularjs-boilerplate` 
is really simple to use, it might help to have an understanding of the tools 
involved here, like Node.js and Grunt and Bower. If you're completely new to highly organized,
...
`angularjs-boilerplate` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone git://github.com/vigneshsakthivel/angularjs-boilerplate my-project-name
$ cd my-project-name
```
...
There are many Bower packages used by `angularjs-boilerplate`, like Twitter Bootstrap
...
Technically, `angularjs-boilerplate` is now ready to go.
...
* `delta:csssrc` - When any `*.css` file in `src/` changes, all the css files are
  concatenated.
...
expected, open the `bin/index.html` file in your browser.
